...
export interface SlamAlertDialogProps {
  ...
  onAction?: () => void;
  /** Overshoot slam duration in ms. Default 500. */
  duration?: number;
  className?: string;
}
 ...
 * 40% with a small vertical offset so it overshoots and settles. Hard offset
 * shadows and borders are drawn with the theme's foreground color, so the whole
 * block inverts correctly in dark mode.
 */
export function SlamAlertDialog({
  ...
  return (
    <AlertDialog.Root>
      ...
      <AlertDialog.Portal>
        <AlertDialog.Backdrop className="fixed inset-0 z-40 bg-black/60 transition-opacity duration-300 data-[starting-style]:opacity-0 data-[ending-style]:opacity-0" />
        <AlertDialog.Popup className="group fixed left-1/2 top-1/2 z-50 w-[calc(100vw-2rem)] max-w-md -translate-x-1/2 -translate-y-1/2 transition-opacity duration-200 data-[starting-style]:opacity-0 data-[ending-style]:opacity-0">
          <div
            style={{
              transitionDuration: `${duration}ms`,
              transitionTimingFunction: "cubic-bezier(0.34,1.56,0.64,1)",
            }}
            className={`border-[3px] border-foreground bg-card p-5 text-card-foreground shadow-[8px_8px_0_0_var(--foreground)] [transition-property:transform] group-data-[starting-style]:[transform:scale(0.4)_translateY(-40px)] group-data-[ending-style]:[transform:scale(0.9)] sm:p-6 ${className}`}
          >
